...
def process_photo(photo_data: str, photo_index: int) -> Tuple[str, List[float], str]:
    ...
    # Try face detection first (for best quality fingerprints)
    try:
        ...
        logger.info(f"Extracted {len(embedding)}-dimensional embedding from photo {photo_index}")
        
        # Generate fingerprint
        fingerprint = generate_fingerprint(embedding)
        logger.info(f"Generated fingerprint for photo {photo_index}: {fingerprint[:16]}... (from face detection)")
        
        return photo_identifier, embedding, fingerprint
        
    except (NoFaceDetectedError, MultipleFacesDetectedError) as e:
        # Face detection failed - fall back to raw image fingerprinting
        logger.warning(f"Face detection failed for photo {photo_index}: {str(e)}")
        logger.info(f"Falling back to raw image fingerprinting for photo {photo_index}")
        
        # Generate a pseudo-embedding from raw image data
        # This creates a 128-dimensional vector from the image hash
        image_hash = hashlib.sha512(image_bytes).digest()
        
        # Convert hash bytes to a list of floats (128 dimensions)
        # Each byte becomes a float between 0 and 1
        embedding = [float(b) / 255.0 for b in image_hash[:128]]
        
        # Pad if necessary (shouldn't be needed with SHA-512, but just in case)
        while len(embedding) < 128:
            embedding.append(0.0)
        
        logger.info(f"Generated {len(embedding)}-dimensional pseudo-embedding from raw image data")
        
        # Generate fingerprint from pseudo-embedding
        fingerprint = generate_fingerprint(embedding)
        logger.info(f"Generated fingerprint for photo {photo_index}: {fingerprint[:16]}... (from raw image)")
