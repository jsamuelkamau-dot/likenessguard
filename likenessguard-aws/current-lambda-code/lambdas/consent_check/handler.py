...
def perform_consent_check(
    ...
    Args:
        ...
        user_id: Optional user ID for self-edit checks
        
    Returns:
        ConsentCheckResponse with decision
        
    Requirements: 9.1, 9.2, 9.3, 10.1, 10.2, 10.3, 10.4, 10.6
    """
    matcher = get_similarity_matcher()
    
    # Find matching fingerprints
    logger.info("Finding matching fingerprints")
    ...
    logger.info("Evaluating consent policy")
    decision, reason_code = evaluate_consent(
        policy=selected_match.consent_policy,
        usage_type=usage_type,
        requester_id=requester_id,
        user_id=user_id or requester_id  # Default to requester_id if user_id not provided
    )
    
    logger.info(f"Consent decision: {decision}, reason: {reason_code}")
