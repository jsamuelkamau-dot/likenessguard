...
input double StopLoss_Pips = 5.0;        // Stop Loss in pips
input double LotSize = 1.0;              // Fixed lot size per pair
input int Magic_Number = 20240009;       // Magic number
...
int OnInit()
{
    Print("=== BCS Liquidity Sweep STANDARD Multi-Currency EA Started ===");
...
{
    Print("=== BCS Liquidity Sweep STANDARD Multi-Currency EA Stopped ===");
}
...
void CheckLiquiditySweep(string symbol, int symbolIndex)
{
    ...
    if(high[1] > swing_high && close[1] < swing_high)
    {
        ...
        {
            Print(symbol, " - STANDARD LOGIC: Opening SELL");
    ...
    if(low[1] < swing_low && close[1] > swing_low)
    {
        ...
        {
            Print(symbol, " - STANDARD LOGIC: Opening BUY");
...
void CheckContinuation(string symbol, int symbolIndex)
{
    ...
    if(sweep.is_buy_signal)
    {
        ...
        if(move >= min_move)
        {
            ...
            Print(symbol, " - STANDARD LOGIC + CONTINUATION: Opening BUY");
            OpenTrade(symbol, true);
            g_symbols[symbolIndex].pending_sweep.is_active = false;
        }
    ...
    else
    {
        ...
        if(move >= min_move)
        {
            ...
            Print(symbol, " - STANDARD LOGIC + CONTINUATION: Opening SELL");
            OpenTrade(symbol, false);
            g_symbols[symbolIndex].pending_sweep.is_active = false;
        }
        else if(move < -min_move)
...
void OpenTrade(string symbol, bool is_buy)
{
    ...
    request.comment = is_buy ? "Standard BUY" : "Standard SELL";
