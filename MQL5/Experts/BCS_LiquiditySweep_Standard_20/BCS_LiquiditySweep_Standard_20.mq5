//+------------------------------------------------------------------+
//|                    BCS_LiquiditySweep_Standard_20.mq5            |
//|              Multi-Currency Standard Liquidity Sweep Trader      |
//|       Low Swept = BUY | High Swept = SELL + Continuation        |
//|                    20 PIP STOP LOSS VERSION                      |
//+------------------------------------------------------------------+
#property copyright "BCS v2.20"
...
input double StopLoss_Pips = 20.0;       // Stop Loss in pips (WIDER)
input double LotSize = 1.0;              // Fixed lot size per pair
input int Magic_Number = 20240010;       // Magic number
...
int OnInit()
{
    Print("=== BCS Liquidity Sweep STANDARD-20 Multi-Currency EA Started ===");
    Print("STANDARD LOGIC: Low Swept = BUY | High Swept = SELL");
    Print("CONTINUATION FILTER: Wait for trend continuation after sweep");
    Print("Take Profit: ", TakeProfit_Pips, " pips");
    Print("Stop Loss: ", StopLoss_Pips, " pips (WIDER for volatility)");
...
{
    Print("=== BCS Liquidity Sweep STANDARD-20 Multi-Currency EA Stopped ===");
}
...
void CheckLiquiditySweep(string symbol, int symbolIndex)
{
    ...
    // STANDARD LOGIC: High swept = SELL (reversal)
    if(high[1] > swing_high && close[1] < swing_high)
    {
        ...
        {
            Print(symbol, " - STANDARD-20 LOGIC: Opening SELL");
            OpenTrade(symbol, false);
        }
        return;
    }
    
    // STANDARD LOGIC: Low swept = BUY (reversal)
    if(low[1] < swing_low && close[1] > swing_low)
    {
        ...
        {
            Print(symbol, " - STANDARD-20 LOGIC: Opening BUY");
...
void CheckContinuation(string symbol, int symbolIndex)
{
    ...
    if(sweep.is_buy_signal)
    {
        ...
        if(move >= min_move)
        {
            ...
            Print(symbol, " - STANDARD-20 LOGIC + CONTINUATION: Opening BUY");
            OpenTrade(symbol, true);
            g_symbols[symbolIndex].pending_sweep.is_active = false;
        }
    ...
    else
    {
        ...
        if(move >= min_move)
        {
            ...
            Print(symbol, " - STANDARD-20 LOGIC + CONTINUATION: Opening SELL");
            OpenTrade(symbol, false);
            g_symbols[symbolIndex].pending_sweep.is_active = false;
        }
        else if(move < -min_move)
...
void OpenTrade(string symbol, bool is_buy)
{
    ...
    request.comment = is_buy ? "Standard20 BUY" : "Standard20 SELL";
