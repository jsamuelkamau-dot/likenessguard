//+------------------------------------------------------------------+
//|                      BCS_LiquiditySweep_Reversal.mq5             |
//|              Multi-Currency Opposite Liquidity Sweep Trader      |
//|    Low Swept = SELL | High Swept = BUY + Stop Loss Reversal     |
//+------------------------------------------------------------------+
#property copyright "BCS v2.50"
...
input double StopLoss_Pips = 7.0;        // Stop Loss in pips
input double LotSize = 1.0;              // Fixed lot size per pair
input int Magic_Number = 20240013;       // Magic number
...
input bool Enable_SL_Reversal = true;    // Enable stop loss reversal
input double Reversal_TP_Pips = 10.0;    // Reversal trade take profit
input double Reversal_SL_Pips = 7.0;     // Reversal trade stop loss
...
struct SymbolData
{
    ...
    ulong last_closed_ticket;
    int last_closed_type;
...
int OnInit()
{
    Print("=== BCS Liquidity Sweep REVERSAL Multi-Currency EA Started ===");
    Print("OPPOSITE LOGIC: Low Swept = SELL | High Swept = BUY");
    Print("REVERSAL FEATURE: Auto-reverse on stop loss hit");
    ...
    Print("Stop Loss Reversal: ", Enable_SL_Reversal ? "ENABLED" : "DISABLED");
    if(Enable_SL_Reversal)
    {
        Print("Reversal TP: ", Reversal_TP_Pips, " pips");
        Print("Reversal SL: ", Reversal_SL_Pips, " pips");
    }
...
void AddSymbol(string symbol)
{
    ...
    g_symbols[g_symbolCount].last_closed_ticket = 0;
    g_symbols[g_symbolCount].last_closed_type = -1;
    g_symbols[g_symbolCount].last_close_time = 0;
    g_symbolCount++;
}
...
{
    Print("=== BCS Liquidity Sweep REVERSAL Multi-Currency EA Stopped ===");
}

//+------------------------------------------------------------------+
//| Trade event handler - detects stop loss hits                      |
//+------------------------------------------------------------------+
void OnTrade()
{
    if(!Enable_SL_Reversal)
    ...
    for(int i = total_deals - 1; i >= 0; i--)
    {
        ...
        // Check if it's an exit deal (OUT)
        ...
        // Check if we already processed this deal
        if(g_symbols[symbolIndex].last_closed_ticket == deal_ticket)
            continue;
        
        // Check if this was a stop loss hit (negative profit)
        if(deal_profit < 0)
        {
            Print(symbol, " - STOP LOSS HIT detected! Profit: ", deal_profit);
            
            // Store deal info
            g_symbols[symbolIndex].last_closed_ticket = deal_ticket;
            g_symbols[symbolIndex].last_closed_type = (int)deal_type;
            g_symbols[symbolIndex].last_close_time = deal_time;
            
            // Open reversal trade (opposite direction)
            bool was_buy = (deal_type == DEAL_TYPE_BUY);
            bool open_sell = was_buy;  // If BUY hit SL, open SELL
            
            Print(symbol, " - Opening REVERSAL trade: ", open_sell ? "SELL" : "BUY");
            OpenReversalTrade(symbol, !open_sell);  // Pass opposite of open_sell
...
//+------------------------------------------------------------------+
//| Open reversal trade after stop loss hit                          |
//+------------------------------------------------------------------+
void OpenReversalTrade(string symbol, bool is_buy)
{
    double price = is_buy ? SymbolInfoDouble(symbol, SYMBOL_ASK) : SymbolInfoDouble(symbol, SYMBOL_BID);
    double point = SymbolInfoDouble(symbol, SYMBOL_POINT);
    int digits = (int)SymbolInfoInteger(symbol, SYMBOL_DIGITS);
    ...
    // Calculate SL and TP using reversal settings
    double sl = is_buy ? 
                price - (Reversal_SL_Pips * pip_value) : 
                price + (Reversal_SL_Pips * pip_value);
                
    double tp = is_buy ? 
                price + (Reversal_TP_Pips * pip_value) : 
                price - (Reversal_TP_Pips * pip_value);
    ...
    request.deviation = 10;
    request.magic = Magic_Number;
    request.comment = is_buy ? "Reversal BUY" : "Reversal SELL";
    
    // Send order
    if(!OrderSend(request, result))
    {
        Print(symbol, " - Reversal OrderSend error: ", GetLastError());
        return;
    }
    
    if(result.retcode == TRADE_RETCODE_DONE)
    {
        Print("=== REVERSAL TRADE OPENED: ", symbol, " ===");
        Print("Direction: ", is_buy ? "BUY" : "SELL");
        Print("Price: ", price);
        Print("SL: ", sl, " (", Reversal_SL_Pips, " pips)");
        Print("TP: ", tp, " (", Reversal_TP_Pips, " pips)");
        Print("Ticket: ", result.order);
    }
    else
    {
        Print(symbol, " - Reversal trade failed: ", result.retcode, " - ", result.comment);
    }
}
