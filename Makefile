...
.PHONY: help install test test-properties lint build deploy clean start-dashboard start-mcp dev-mock
...
test: ## Run all unit tests (no AWS needed)
...
dev-mock: ## Run full mock test suite — property tests + unit tests (no AWS needed)
	cd likenessguard-aws && python -m pytest src/tests/test_v2_properties.py -v
	cd likenessguard-aws && python -m pytest tests/ -v

lint: ## Lint Python code (no AWS needed)
	flake8 likenessguard-aws/src/lambdas/ --max-line-length=150 --ignore=E501,W503,E402,F401,F811

build: ## Build SAM application (requires AWS)
	cd likenessguard-aws && sam build --template-file infrastructure/cloudformation-v2.yaml

deploy: build ## Deploy full stack to AWS (requires AWS)
...
post-deploy: ## Run post-deployment setup — OpenSearch index + JWKS (requires AWS)
	cd likenessguard-aws && python scripts/deploy_opensearch_index.py
	cd likenessguard-aws && python scripts/publish_jwks.py

start-dashboard: ## Start React dashboard locally (no AWS needed)
	cd likenessguard-dashboard && npm run dev

start-mcp: ## Start MCP server for Claude.ai (no AWS needed)
	cd likenessguard-mcp && python server.py

clean: ## Clean build artifacts (no AWS needed)
